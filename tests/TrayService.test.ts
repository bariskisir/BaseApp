...
const electronMocks = vi.hoisted(() => {
  class MockTray {
    ...
    /** Captures tray event listeners so tests can trigger native interactions. */
...
vi.mock('electron', () => ({
  app: {
    isPackaged: false,
    /** Returns the deterministic development root used to resolve the tray icon. */
    getAppPath: () => 'C:\\repo',
  ...
  nativeImage: {
    createFromPath: vi.fn(() => ({
      /** Reflects the scenario-controlled icon loading result. */
      isEmpty: () => electronMocks.imageEmpty,
      resize: vi.fn(() => ({ resized: true })),
    })),
  },
  Tray: class extends electronMocks.MockTray {
    /** Registers each mocked native Tray instance for assertions. */
    public constructor() {
      super()
...
describe('TrayService', () => {
  ...
  it('keeps tray behavior disabled on Linux even when persisted preferences enable it', () => {
    const service = new TrayService(
      createWindow(),
      { showTrayIcon: true, minimizeToTrayOnClose: true },
      createLogger(),
