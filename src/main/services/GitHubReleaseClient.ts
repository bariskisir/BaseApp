/**
 * Retrieves published releases and downloads verified installers from GitHub.
 */
...
import { z } from 'zod'
import { APP_NAME, APP_REPO, APP_SLUG, APP_USER_AGENT } from '@shared/appInfo'

const RELEASES_API_URL = `https://api.github.com/repos/${APP_REPO}/releases/latest`
const GITHUB_ORIGIN = 'https://github.com'
const RELEASE_DOWNLOAD_PATH_PREFIX = `/${APP_REPO}/releases/download/`.toLowerCase()
...
/** Trusted release asset metadata required for an installer download. */
...
/** Stable GitHub release metadata consumed by the update coordinator. */
...
/** Local file details produced by a successfully verified installer download. */
export interface DownloadedRelease {
  filePath: string
  sha256: string
}

/** Receives integer completion percentages while an installer is downloaded. */
...
export const selectWindowsInstaller = (
  ...
  const expectedName = `${APP_SLUG}-${release.version}-windows-${releaseArchitecture}-setup.exe`
...
/** Retrieves stable GitHub releases and safely downloads their installer assets. */
export default class GitHubReleaseClient {
  ...
  /** Requests and validates the latest release from the configured GitHub repository. */
  ...
  /** Validates one GitHub REST response and converts it into the internal release contract. */
  private async parseApiRelease(response: Response): Promise<GitHubRelease> {
    ...
    return {
      version,
      name: parsed.name ?? `${APP_NAME} v${version}`,
