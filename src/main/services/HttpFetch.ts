/**
 * Routes outbound HTTPS requests through Electron's network stack when available.
 *
 ...
 * The Chromium stack trusts the OS certificate store, so update and telemetry calls keep
 * working behind TLS-intercepting antivirus software or corporate proxies whose root CA is
 * installed system-wide but unknown to Node's bundled CA list.
 */
 ...
 *
 * Shared by update downloads and telemetry so every outbound HTTPS call trusts the OS
 * certificate store inside Electron.
 *
