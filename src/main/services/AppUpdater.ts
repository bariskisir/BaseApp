/**
 * Coordinates GitHub update checks, verified downloads, and attended or unattended installation.
 */
...
export interface UpdateRuntime {
  ...
  /** Exits the running Electron application after installer launch. */
  quit(): void
  /** Starts the downloaded installer and resolves after the operating system accepts it. */
...
export interface UpdateLogger {
  /** Records an updater failure with optional diagnostic detail. */
  error(module: string, message: string, details?: unknown): void
  /** Records an updater lifecycle event with optional metadata. */
...
export interface UpdateClient {
  /** Retrieves the latest stable release metadata. */
  getLatestRelease(): Promise<GitHubRelease>
  /** Downloads and verifies one installer while reporting progress. */
  downloadInstaller(
...
const createRuntime = (): UpdateRuntime => ({
  ...
  /** Exits the active Electron process after a successful installer launch. */
  quit: () => app.quit(),
  launchInstaller,
})

/** Coordinates one update lifecycle while preventing overlapping release checks. */
export default class AppUpdater {
  ...
  /** Checks GitHub and downloads a matching installer only for packaged Windows builds. */
  ...
  private async performUpdateCheck(): Promise<void> {
    ...
    try {
      ...
      if (this.runtime.platform !== 'win32' || !this.runtime.isPackaged) {
        this.emit({
          state: 'available',
